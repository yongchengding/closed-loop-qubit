...
agent.restore(directory='pretrained',filename='agent-518770')
...
baselinez = np.load("Sz_DRL_relaxation.npy")
baseliney = np.load("Sy_DRL_relaxation.npy")  
...
plt.plot(tlist,Symean,color='darkred',label=r'$\overline{\langle\hat{\sigma_y}\rangle}$, NE, OM')
plt.plot(tlist,Szmean,color='darkblue',label=r'$\overline{\langle\hat{\sigma_z}\rangle}$, NE, OM')
plt.plot(tlist,Sxmean,color='darkgreen',label=r'$\overline{\langle\hat{\sigma_x}\rangle}$, NE, OM')
plt.plot(tlist,baseliney,color='darkred',linewidth=1,linestyle='--',label=r'$\overline{\langle\hat{\sigma_y}\rangle}$, OE, OM')
plt.plot(tlist,baselinez,color='darkblue',linewidth=1,linestyle='--',label=r'$\overline{\langle\hat{\sigma_z}\rangle}$, OE, OM')
...
fig.savefig('rhodrl_shift.pdf',dpi=800,bbox_inches='tight',format='pdf')
...
ax.tick_params(axis='both', labelsize=20)
fig.savefig('omegadrl_shift.pdf',dpi=800,bbox_inches='tight',format='pdf')

#np.save("Sx_DRL_before_transfer.npy",Sxmean)
#np.save("Sy_DRL_before_transfer.npy",Symean)
#np.save("Sz_DRL_before_transfer.npy",Szmean)
