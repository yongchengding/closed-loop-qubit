...
def three(A,gamma,H,rho,dt):
    C = np.sqrt(gamma)*A
    Cdag = np.transpose(np.conjugate(C))
    ddt = dt/100
    for i in range(100):
        master_term = -1j*(np.dot(H,rho)-np.dot(rho,H))
        lind_term1 = 0.5*(2*np.dot(np.dot(C,rho),Cdag)-np.dot(np.dot(rho,Cdag),C)-np.dot(np.dot(Cdag,C),rho))
        lind_term2 = -gamma*(rho - np.diag(np.diag(rho)))   
        lhs = master_term + lind_term1 + lind_term2
        rho = rho + lhs*ddt        
    return rho
