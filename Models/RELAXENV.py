...
class WEAKMEASUREMENT(Environment):
    ...
    def reset(self):
        ...
        self.STATES=np.array([0,(np.random.normal(loc=1, scale=self.sigma, size=None)+50)/100,1,0,0,0,0])
        #Delta q0 rho11 rho12 rho21 rho22  t=0
    ...
    def execute(self, actions):
        ...
        """
        detuning = actions[0]
        delta = (2*2*detuning-2)*self.Omega # Delta in [-2*self.Omega,2*self.Omega] 
        omega = self.Omega
        ...
        self.rho = relaxtion(self.sx,self.gamma,TLS,self.rho,self.dt)
        ...
        self.timestep += 1
        self.STATES = np.array([actions[0], q0, abs(self.rho[0,0]), abs(self.rho[0,1]), abs(self.rho[1,0]), abs(self.rho[1,1]),self.timestep/100])

        reward = 0  
        
        #'''
        if abs(self.rho[1,1]) >= 0.98:
            self.escape += 1
        else:
            self.escape = 0
        if self.escape == 4:
