...
from mixedstate import primerotrace, seguiendotrace, coupling, calfidelity, perturb, operatorQ, operatorP, createpointer, dephasing
...
np.random.seed(1)
Omega = np.pi
TLS = 0.5*(np.array([[0,Omega],[Omega,0]],dtype=complex))
tf = np.pi/Omega
dt = 0.01
steps = int(tf/dt)
...
for i in range(steps):
    rho = dephasing(gamma,TLS,rho,dt)
    #print(rho)
...
#fig.savefig('pipulsedephasing.pdf',dpi=800,bbox_inches='tight',format='pdf')
np.save("szmean_dephasing.npy",Szmean)
np.save("symean_dephasing.npy",Symean)
