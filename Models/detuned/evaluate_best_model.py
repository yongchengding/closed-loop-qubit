...
agent.restore(directory='best-model',filename='agent-132633')
...
baselinez = np.load("szdetuned.npy")
baseliney = np.load("sydetuned.npy")  
baselinex = np.load("sxdetuned.npy")
...
plt.plot(tlist,Symean,color='darkred',label=r'$\overline{\langle\hat{\sigma_y}\rangle}$, RL')
plt.plot(tlist,Szmean,color='darkblue',label=r'$\overline{\langle\hat{\sigma_z}\rangle}$, RL')
plt.plot(tlist,Sxmean,color='darkgreen',label=r'$\overline{\langle\hat{\sigma_x}\rangle}$, RL')
plt.plot(tlist,baseliney[1:101],color='darkred',linewidth=1,linestyle='--',label=r'$\overline{\langle\hat{\sigma_y}\rangle}$, $\pi$-pulse')
plt.plot(tlist,baselinez[1:101],color='darkblue',linewidth=1,linestyle='--',label=r'$\overline{\langle\hat{\sigma_z}\rangle}$, $\pi$-pulse')
plt.plot(tlist,baselinex[1:101],color='darkgreen',linewidth=1,linestyle='--',label=r'$\overline{\langle\hat{\sigma_x}\rangle}$, $\pi$-pulse')
...
fig.savefig('rhodrl_detunded.pdf',dpi=800,bbox_inches='tight',format='pdf')
...
ax.tick_params(axis='both', labelsize=20)
fig.savefig('omegadrl_detunded.pdf',dpi=800,bbox_inches='tight',format='pdf')
